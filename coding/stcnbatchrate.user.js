...
// @updateURL    https://sffxzzp.coding.net/p/userscripts/d/userscripts/git/raw/master/stcnbatchrate.user.js
// @grant        unsafeWindow
// ==/UserScript==
    ...
    var stcnbr = (function () {
        ...
        stcnbr.prototype.mass = function (score, reason) {
            ...
            for (var i=0; i<postlist.children.length; i++) {
                ...
                if (post.id.substr(0, 5) == "post_") {
                    this.rate(formhash, atid, post.id.substr(5), "score1="+score, reason);
                    queue += 1;
                }
            }
        }
        stcnbr.prototype.showPanel = function (x, y) {
            ...
            var panel = util.createElement({
                ...
                content: {
                    id: "mass_rate",
                    style: "position: absolute; background-color: white; top: "+(x+20)+"px; left: "+(y-100)+"px;"
                },
                html: "<input id=\"mass_score\" placeholder=\"请输入加分数值\"/><br><input id=\"mass_reason\" placeholder=\"请输入加分原因\"/><br><button id=\"mass_ok\">确定</button>&nbsp;&nbsp;&nbsp;&nbsp;<button id=\"mass_close\">关闭</button>"
            });
        ...
        stcnbr.prototype.run = function () {
            ...
            var mrBtn = util.createElement({node: "a", content: {href: "javascript:;", style: "float: right;"}, html: "批量评分"});
